...
func DeleteUserJsonHandler(ctx echo.Context, r *services.Registrar) error {
	return NewDeleteUserHandler(ctx, r).Handle().JSON()
}
...
func (h *DeleteUserHandler) Handle() handlers.IHandler {
	...
	if err = h.r.AuthService.CheckToken(jwt_token.Raw); err != nil {
		return handlers.Lock(h, 401, err)
	}
	if user, err = h.r.UserService.Get(userID); err != nil {
	...
	if err = h.r.UserService.Remove(deleteId); err != nil {
		return handlers.Lock(h, 404, err)
	}
	return h
}
